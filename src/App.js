...
const ThemedApp = () => {
  ...
  const [filterCriteria] = useState({ startDate: '', endDate: '', category: '' });
  ...
  const scheduleNotificationOnAppOpen = useCallback(() => {
    if (!isMobileDevice()) return;

    showNotification(); // Show notification immediately when the app is opened
  }, [isMobileDevice]);

  const showNotification = async () => {
    if ('serviceWorker' in navigator) {
      const registration = await navigator.serviceWorker.ready;
      registration.showNotification('Finance Tracker', {
        body: 'Welcome back! Check your finances today.',
        icon: '/icon.png',
        badge: '/badge.png',
      });
    }
    ...
    if ('Notification' in window && 'serviceWorker' in navigator) {
      Notification.requestPermission().then((permission) => {
        if (permission === 'granted') {
          ...
          scheduleNotificationOnAppOpen(); // Trigger the notification when app opens
        }
      });
    }
  }, [scheduleNotificationOnAppOpen]);
